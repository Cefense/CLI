...
Every command takes `--agent` and prints one line of JSON on stdout. Branch on `error.code`, never on `error.message`. `cf agent check --agent` says whether you can proceed and what is blocking you; `cf agent schema --agent` prints the whole command surface, generated by the CLI itself. `cf proof run` replays the evidence that produced a finding against the patch that claims to close it, and a `refuted` verdict blocks publishing. `cf fix publish` opens a real pull request and `cf fix merge` lands it on the default branch, so ask the user before running either, and treat merging as a separate ask from opening. `cf proof attest` records under the user's name that a leaked credential was rotated, so ask them too, and `cf proof env set` stores a value a proof boots the app with, so ask before storing one. Each finding carries `reachability` (work `reachable` first) and `status`, the same word the workspace's status ring shows. `cf settings` writes the user's scanning policy and `cf triage` records their judgement about a finding under their name, so ask before either.
